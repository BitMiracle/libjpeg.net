...
namespace dJpeg
{
    ...
    class bmp_dest_struct : djpeg_dest_struct
    {
        ...
        private uint data_width;  /* bytes per row */
        private uint row_width;       /* physical width of one row in the BMP file */
        private int pad_bytes;      /* number of padding bytes needed per row */
        private uint cur_output_row;  /* next row# to write to virtual array */
        ...
        public bmp_dest_struct(jpeg_decompress_struct cinfo, bool is_os2)
        {
            ...
            buffer = jpeg_common_struct.AllocJpegSamples(row_width, (uint)1);
            buffer_height = 1;
        }
        ...
        /// </summary>
        public override void put_pixel_rows(uint rows_supplied)
        {
            if (m_putGrayRows)
        ...
        public override void finish_output()
        {
            ...
            for (uint row = cinfo.Output_height; row > 0; row--)
            {
                ...
                byte[][] image_ptr = whole_image.access_virt_sarray(row - 1, (uint)1);
                int imageIndex = 0;
        ...
        /// </summary>
        private void put_24bit_rows(uint rows_supplied)
        {
            /* Access next row in virtual array */
            byte[][] image_ptr = whole_image.access_virt_sarray(cur_output_row, (uint)1);
            cur_output_row++;
        ...
        /// </summary>
        private void put_gray_rows(uint rows_supplied)
        {
            /* Access next row in virtual array */
            byte[][] image_ptr = whole_image.access_virt_sarray(cur_output_row, (uint)1);
            cur_output_row++;
    ...
    class bmp_dest_struct : djpeg_dest_struct
    {
        ...
        public bmp_dest_struct(Decompressor decompressor, bool is_os2)
        {
            ...
            {
                decompressor.ClassicDecompressor.ERREXIT((int)ADDON_MESSAGE_CODE.JERR_BMP_COLORSPACE);
            }
            ...
            /* Determine width of rows in the BMP file (padded to 4-byte boundary). */
            row_width = (uint)(decompressor.OutputWidth * decompressor.OutputComponents);
            data_width = row_width;
            while ((row_width & 3) != 0)
                row_width++;
            ...
            whole_image = new jvirt_sarray_control(cinfo, false, row_width, (uint)decompressor.OutputHeight);
            cur_output_row = 0;
            if (cinfo.Progress != null)
            {
                cdjpeg_progress_mgr progress = (cdjpeg_progress_mgr)cinfo.Progress;
        ...
        public override void finish_output()
        {
            ...
            for (uint row = cinfo.Output_height; row > 0; row--)
            {
                if (progress != null)
                {
                    progress.Pass_counter = (long)(decompressor.OutputHeight - row);
                    progress.Pass_limit = (long)decompressor.OutputHeight;
                    progress.progress_monitor();
                }
        ...
        private void write_bmp_header()
        {
            ...
            {
                decompressor.ClassicDecompressor.TRACEMS(0, (int)J_MESSAGE_CODE.JERR_FILE_WRITE, e.Message);
                decompressor.ClassicDecompressor.ERREXIT((int)J_MESSAGE_CODE.JERR_FILE_WRITE);
            }
        ...
        private void write_colormap(int map_colors, int map_entry_size)
        {
            ...
            /* Pad colormap with zeros to ensure specified number of colormap entries */
            if (i > map_colors)
                decompressor.ClassicDecompressor.ERREXIT((int)ADDON_MESSAGE_CODE.JERR_TOO_MANY_COLORS, i);
